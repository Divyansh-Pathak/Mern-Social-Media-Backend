...
function App() {
  ...
  function runGetUser (condition) {
    ...
    if(condition){
      setLoading(true);
      // console.log("aaya to h rungetuser me");
      userRoute.getUser().then((data) => {
        // console.log("get User Bhi chal gya");
        if(data === 'noUser'){
          setUser('noUser');
        }else{
          setUser(data);
    ...
    <Router>
      <userContext.Provider value={user}>
        ...
        <Header loggedInState={(val) => setLoggedIn(val)} />
          <Switch>
            {/* <Route exact path='/' component={() => <Loading isLoading={isLoading}/>} /> */}
            <Route exact path="/" component={() => {
              if (isLoggedIn === 'loggedOut') return <Redirect to='/login' />
              else if(isLoggedIn === "loggedIn") return <Redirect to='/home' />
              else return <Loading isLoading={isLoading}/>
            }} />
            {/* <Route exact path="/home" component={Home} /> */}
            <Route exact path="/login" component={() => <Login reRunGetUser={runGetUser} 
            loggedInState={(val)=>setLoggedIn(val)} />} />
